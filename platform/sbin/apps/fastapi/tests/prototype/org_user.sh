...
function list_workflow() {
  ...
  set -x
  curl --insecure -s -X 'GET' \
  "https://${fastapi_url}/organization/workflow/template" \
  -H 'accept: application/json' \
  -H "token: ${token}" \
  -H 'Content-Type: application/json'
  set +x
...
function submit_workflow() {
  ...
  workflow_name=$3
  workflow_input_dir=$4
  workflow_cluster=$5
  workflow_parameters_cpus=$6
  set -x
  curl --insecure -s -X 'POST' \
  ...
  -H "token: ${token}" \
  -H 'Content-Type: application/json' \
  -d "{\"workflow_name\": \"${workflow_name}\", \"workflow_input_dir\": \"${workflow_input_dir}\", \"workflow_placement\": {\"cluster\": \"${workflow_cluster}\"}, \"workflow_parameters\": {\"cpus\": \"${workflow_parameters_cpus}\", \"memory\": \"1024\"}}"
...
# clear
# echo "### TEST: Submit dummy workflow"
# response=$(submit_workflow "${token}" "1" "dummy-workflow" "/basicuser/wf1-input" "main_cluster" "1")
# echo "${response}" | jq
# read $wait_demo

# clear
# echo "### TEST: Submit openfoam workflow"
# response=$(submit_workflow "${token}" "2" "openfoam-workflow" "/basicuser/wf2-input" "main_cluster" "2")
# echo "${response}" | jq
# read $wait_demo

# clear
# echo "### TEST: Submit tensorflow workflow"
# response=$(submit_workflow "${token}" "3" "tensorflow-workflow" "/basicuser/wf3-input" "main_cluster" "2")
# echo "${response}" | jq
# read $wait_demo

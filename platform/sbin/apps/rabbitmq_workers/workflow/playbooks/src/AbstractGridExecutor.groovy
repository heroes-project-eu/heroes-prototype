...
abstract class AbstractGridExecutor extends Executor {
    ...
    protected Map<String,QueueStatus> getQueueStatus0(queue) {
        ...
        log.debug("getQueueStatus0: start")
        // log.debug "[${name.toUpperCase()}] getting queue ${queue?"($queue) ":''}status > cmd: ${cmd.join(' ')}"
        log.debug "${cmd.join(' ')}"

        // cmd.eachWithIndex { item, index -> log.debug "$item in position $index" }
    ...
    boolean checkActiveStatus( jobId, queue ) {
        ...
        // -- fetch the queue status
        log.debug("checkActiveStatus: start")
        final status = getQueueStatus(queue)

        log.debug("checkActiveStatus: status received")

        if( status == null ) { // no data is returned, so return true
            log.debug("checkActiveStatus: status null")
            return true
        }
        ...
        log.debug("checkActiveStatus: end")
        return result
    }

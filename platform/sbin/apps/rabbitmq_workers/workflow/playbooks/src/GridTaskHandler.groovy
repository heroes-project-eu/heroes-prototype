...
class GridTaskHandler extends TaskHandler {
    ...
    protected Integer readExitStatus() {
        ...
        def cmd = script + " " + "status" + " " + task.workDir + " " + clusterConfig
        Process proc = cmd.execute()
        proc.waitForProcessOutput(sout, serr)*/

        // Thread.sleep(5000)

        log.debug("COPY FILES START")
	      // Transfer output data from cluster to minio and from minio to local

        if (!dataTransferServerToMinio() || !dataTransferMinioToLocal())
            return null
        else
            log.trace("file transfer after completion finished")


        log.debug("COPY FILES END")
        ...
        if( !exitFile || !(exitAttrs=FileHelper.readAttributes(exitFile)) || !exitAttrs.lastModifiedTime()?.toMillis() ) {
            ...
            if( active ) {
                ...
                log.debug("Wait end of task")
                exitTimestampMillis1 = 0
                return null
            }
            else {
                log.debug("TASK NOT ACTIVE: wait 60s")
                Thread.sleep(60000)
                log.debug("TASK NOT ACTIVE: try last data movement")
                if (!dataTransferServerToMinio() || !dataTransferMinioToLocal())
                    return null
                else
                    log.trace("file transfer after completion finished")
                log.debug("TASK NOT ACTIVE: end of last data movement")
            }
    ...
    public TraceRecord getTraceRecord() {
      ...
      // Uses Rclone to send data from the cluster to minio
      protected boolean dataTransferServerToMinio() {

          def userWorkDir = task.workDirStr.split("/").dropRight(2).join("/")
          def cmd = "rclone sync " + task.config.getCluster()[0] + ":" + userWorkDir + " " + task.config.getMinio() + ":" + task.config.getBucket() + "/" + userWorkDir.split("/").drop(5).join("/") + " --config " + task.config.getData() + " --exclude " + task.container.split("/").drop(7).join()

          println("dataTransferServerToMinio")
          log.debug("dataTransferServerToMinio")

          // The workdir is NFS, then the metadata are not sync. The ls is here to sync the data.
          def cmd_sync = "rclone ls " + task.config.getCluster()[0] + ":" + userWorkDir + " --config " + task.config.getData()
          def result_cmd = runProc(cmd_sync)

          return runProc(cmd)
      }

      // Uses Rclone to send the data from minio to nextflow executor
      protected boolean dataTransferMinioToLocal() {

          def userWorkDir = task.workDirStr.split("/").dropRight(2).join("/")
          def cmd = "rclone sync " + task.config.getMinio() + ":" + task.config.getBucket() + "/" + userWorkDir.split("/").drop(5).join("/") + " " + userWorkDir + " --config " + task.config.getData() + " --exclude " + task.container.split("/").drop(7).join()

          println("dataTransferMinioToLocal")
          log.debug("dataTransferMinioToLocal")
      ...
      // General run command function to do not repeat the code
      protected boolean runProc(String cmd) {

          log.debug("[runProc] start")
          log.debug(cmd)
          def sout = new StringBuilder(), serr = new StringBuilder()
